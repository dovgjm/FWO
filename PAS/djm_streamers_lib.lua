...
-- @provides
--  [nomain] .
--  [main] data/djm_add custom streamer.lua
--  [main] data/djm_add flutter.lua
--  [main] data/djm_add green streamer 2s.lua
--  [main] data/djm_add green streamer 3s.lua
--  [main] data/djm_add green streamer custom length.lua
--  [main] data/djm_add punch.lua
--  [main] data/djm_add red streamer 2s.lua
--  [main] data/djm_add red streamer 3s.lua
--  [main] data/djm_add white streamer 2s.lua
--  [main] data/djm_add white streamer 3s.lua
--  [main] data/djm_add yellow streamer 2s.lua
--  [main] data/djm_add yellow streamer 3s.lua
--  [main] data/djm_load punches and streamers from CSV.lua
--  [main] data/djm_save punches and streamers to csv.lua
--  [main] data/djm_show streamers list.lua
--  [main] data/djm_streamer settings disable end inside.lua
--  [main] data/djm_streamer settings disable start inside.lua
--  [main] data/djm_streamer settings enable end inside.lua
--  [main] data/djm_streamer settings enable start inside.lua
--  [main] data/djm_streamer settings toggle end inside.lua
--  [main] data/djm_streamer settings toggle start inside.lua
--  [main] data/djm_update click track.lua
--  [main] data/djm_update punches and streamers from markers.lua
--  [nomain] data/settings.lua
--  [nomain data] data/FX/*.txt
--  [nomain data] toolbar/FilmSync.ReaperMenu
...
function insertPunch(position, punchNum, length, color, undo)
  if undo == nil then undo = true end -- default
...
-- insert midi item
--  local punchItem = reaper.CreateNewMIDIItemInProj(punchTrack, position, position+length, false)
    local punchItem = reaper.CreateNewMIDIItemInProj(punchTrack, position, position+2*df, false)  -- Needs to be framerate dependant on the length
	
	-- calculate color
	println("color: " .. color)
	local r, g, b = getColorValues(color)
	
	reaper.SetMediaItemInfo_Value(punchItem, "I_CUSTOMCOLOR", reaper.ColorToNative(r*255, g*255, b*255)|0x1000000)
	
	-- keep item fixed in time
	reaper.SetMediaItemInfo_Value(punchItem, "C_BEATATTACHMODE", 0)
	reaper.SetMediaItemInfo_Value(punchItem, "B_LOOPSRC", 0)
	
	
	-- apply vfx
	if(punchItem) then
	  addPunchFX(punchItem, "punch_circle_FX.txt", true, r, g, b)
	else
	  println("Error creating empty MIDI item: " .. punchItem)
	end
...
function addPunchFX(trackOrItem, FX, isItem, r, g, b)
  -- read cached FX chunk from text file and add to track chunk
  local retval, trackOrItemChunk
  if not isItem then
    println("Trying to get Track state chunk...")
  ...
  if retval then
    ...
    if file then
      ...
      if isItem then
        ...
        -- set thickness, replace colors, set duration, set start/end inside
        fxChunk = fxChunk:gsub("CODEPARM .*$", "CODEPARM "  .. r .. " " .. g .. " " .. b .. " " .. alpha .. " " .. alpha)
      end
      fxChunk = fxChunk .. 
[[  FXID ]] .. fxGUID .. [[
  WAK 0
...
>]]
      ...
      if not isItem then
        ...
        reaper.SetItemStateChunk(trackOrItem, trackOrItemChunk)
        reaper.UpdateItemInProject(trackOrItem)
    println("setItemStateChunk")
      end
    else
      println("Could not read FX file " .. FX)
    end
  else
    println("Could not create punch FX " .. FX)
  end
end
...
function insertStreamer(position, length, color, showPunch, undo)
	...
	if showPunch then
		insertPunch(position, m, false, color)
	end
...
--    if (markerName == "P") or (markerName == "P ") then
    if(markerName:find("P") == 1 or markerName:find("P ") == 1) then
		...
		insertPunch(position, m, false, color, undo)
	  end
	end
